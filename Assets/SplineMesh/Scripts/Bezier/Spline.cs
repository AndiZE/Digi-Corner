...
namespace SplineMesh
{
    ...
    public class Spline : MonoBehaviour
    {
        ...
        public Vector3[] IsInsideCollision(Collider collider, int stepsPerUnityUnit = 10)
        {
            ...
            for (int i = 0; i < stepLenght; i++)
            {
                if (collider.bounds.Contains(GetSampleAtDistance(i * stepSize).location + rootpos))
                {
                    positions.Add(GetSampleAtDistance(i * stepSize).location + rootpos);
                }
            }
